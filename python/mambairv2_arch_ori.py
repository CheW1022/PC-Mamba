...
from tools.data_loader import load_data_Charles
...
class dwconv(nn.Module):
    def __init__(self, hidden_features, kernel_size=5):
        ...
        #         stride=1,
        #         wt_levels=1,         # 小波分解层数，按需调整
        #         wt_type='db1'        # 小波基类型，如'haar'、'db2'
        #     ),
        #     nn.GELU())
    ...
    def forward(self, x, x_size):
        ...
        # if DEBUG:
        #     visualize_feature(x, "Before depthwise_conv", n_channels=1)  # 显示前4个通道

        x = self.depthwise_conv(x)

        # if DEBUG:
        #     visualize_feature(x, "After depthwise_conv", n_channels=1)
...
class MambaIRv2(nn.Module):
    def __init__(self,
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            if num_in_ch != num_out_ch:
                self.skip_conv = nn.Conv2d(num_in_ch, num_out_ch, kernel_size=1)
            else:
                self.skip_conv = None

            self.conv_last = nn.Conv2d(embed_dim, num_out_ch, 3, 1, 1)
            self.conv_one = nn.Conv2d(in_channels=num_out_ch, out_channels=num_out_ch, kernel_size=1)
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            a1 = self.conv_last(res)
            # x = x + a1
            if self.skip_conv is not None:
                x_res = self.skip_conv(x)
            else:
                x_res = x
            x = x_res + a1
        ...
        # unpadding
        # x = x[..., :h_ori * self.upscale, :w_ori * self.upscale]   ###upsample
        x = x[..., :h_ori , :w_ori]
...
if __name__ == '__main__':
    ...
    input_tensor = torch.randn(batchsize, 2, 132, 132).to(device)
    # input_tensor = torch.randn(batchsize, 2, 512, 512).to(device)
    ...
    #     LABEL_start = 5
    #     LABEL_end =6
    # file_path = "/DATA2023/linyr/data/T2T2star/simusub/train/train_0001.Charles"
    # device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")
    # input_data, label_data = load_data_Charles(file_path, Config)
    # print(type(input_data), input_data.shape)
    # input_tensor = torch.from_numpy(input_data.transpose(2, 0, 1)).unsqueeze(0).float().to(device)  # [1, C, H, W]
    # print(type(input_tensor), input_tensor.shape)
    #
    # net = MambaIRv2(img_size=256,
    #                  patch_size=1,
    #                  in_chans=4,
    #                  out_chans=2)
    #
